...
public sealed class ReturnOrderCommandHandler(
    IOrderRepository orderRepository,
    IProductRepository productRepository)
    : ICommandHandler<ReturnOrderCommand, Result<Order>>
{
    public async ValueTask<Result<Order>> Handle(ReturnOrderCommand command, CancellationToken cancellationToken)
    {
        ...
        var returnResult = order.Return(command.Reason, products);
        if (!returnResult.TryGetValue(out var returnedOrder))
        ...
        var saveOrderResult = await orderRepository.SaveAsync(returnedOrder, cancellationToken);
