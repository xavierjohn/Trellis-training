...
public class OrdersController(ISender sender) : ControllerBase
{
    ...
    [ProducesResponseType(StatusCodes.Status403Forbidden)]
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    public async Task<ActionResult<OrderDto>> ReturnOrder(
        string id,
        [FromBody] ReturnOrderRequest request,
        CancellationToken cancellationToken) =>
        await OrderId.TryCreate(id)
            .Combine(ReturnReason.TryCreate(request.Reason))
            .Map((orderId, reason) => new ReturnOrderCommand(orderId, reason))
            .BindAsync(command => sender.Send(command, cancellationToken))
            .MapAsync(MapToDto)
            .ToActionResultAsync(this);
    ...
    private static OrderDto MapToDto(Order o) => new(
        ...
        o.DeliveredAt.HasValue ? o.DeliveredAt.Value : null,
        o.ReturnedAt.HasValue ? o.ReturnedAt.Value : null,
