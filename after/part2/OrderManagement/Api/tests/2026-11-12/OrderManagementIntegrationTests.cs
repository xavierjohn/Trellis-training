...
public class OrderManagementIntegrationTests
{
    ...
    private async Task<(HttpClient client, OrderResponse order)> CreateDeliveredOrderAsync(string actorId = "return-user")
    {
        var client = CreateClient(actorId, AllPermissions);
        var email = $"return-{Guid.NewGuid():N}@example.com";
        var customer = await CreateTestCustomer(client, email);
        var sku = $"RT-{Guid.NewGuid().ToString("N")[..6]}".ToUpper(CultureInfo.InvariantCulture);
        var product = await CreateTestProduct(client, sku);

        await client.PostAsJsonAsync($"api/Products/{product.Id}/stock-additions?{V}",
            new { quantity = 100 }, TestContext.Current.CancellationToken);
        ...
        await client.PostAsync($"api/Orders/{order.Id}/submission?{V}", null, TestContext.Current.CancellationToken);
        await client.PostAsync($"api/Orders/{order.Id}/approval?{V}", null, TestContext.Current.CancellationToken);
        await client.PostAsync($"api/Orders/{order.Id}/shipment?{V}", null, TestContext.Current.CancellationToken);
        var deliverResp = await client.PostAsync($"api/Orders/{order.Id}/delivery?{V}", null, TestContext.Current.CancellationToken);
    ...
    [Fact]
    public async Task ReturnOrder_DeliveredOrder_Returns200()
    {
        var (client, order) = await CreateDeliveredOrderAsync();

        var returnResp = await client.PostAsJsonAsync($"api/Orders/{order.Id}/return?{V}",
            new { reason = "Product was damaged during shipping" }, TestContext.Current.CancellationToken);

        returnResp.StatusCode.Should().Be(HttpStatusCode.OK);
        var returned = await returnResp.Content.ReadAsAsyncWithAssertion<OrderResponse>();
        returned.Status.Should().Be("Returned");
    }

    [Fact]
    public async Task ReturnOrder_NotDelivered_Returns400()
    {
        var client = CreateClient("return-invalid", AllPermissions);
        var email = $"rni-{Guid.NewGuid():N}@example.com";
        var customer = await CreateTestCustomer(client, email);
        var sku = $"RN-{Guid.NewGuid().ToString("N")[..6]}".ToUpper(CultureInfo.InvariantCulture);
        var product = await CreateTestProduct(client, sku);
        ...
        returnResp.StatusCode.Should().Be(HttpStatusCode.BadRequest);
    }

    [Fact]
    public async Task ReturnOrder_MissingPermission_Returns403()
    {
        var (_, order) = await CreateDeliveredOrderAsync("return-perm-user");

        var noPermClient = CreateClient("return-perm-user", "orders:read");
        var returnResp = await noPermClient.PostAsJsonAsync($"api/Orders/{order.Id}/return?{V}",
            new { reason = "Product was damaged during shipping" }, TestContext.Current.CancellationToken);

        returnResp.StatusCode.Should().Be(HttpStatusCode.Forbidden);
    }

    [Fact]
    public async Task ReturnOrder_InvalidReason_Returns400()
    {
        var (client, order) = await CreateDeliveredOrderAsync("return-reason-user");

        var returnResp = await client.PostAsJsonAsync($"api/Orders/{order.Id}/return?{V}",
            new { reason = "short" }, TestContext.Current.CancellationToken);
