...
| 3 | **Business failure → HTTP 500 + leaked detail** (`Bug3_InsufficientStock_Returns500WithLeakedMessage_NotA422`) | insufficient stock is `throw`-n and the endpoint's catch-all returns `500` with the internal message | `Error.InvalidInput.ForRule(...)` maps to a **4xx client error** + RFC 9457 ProblemDetails, no exception or stack trace (Trellis's default for `InvalidInput` is 422; the OM spec's error table lists 400 — either way a client error, never a 500) |
